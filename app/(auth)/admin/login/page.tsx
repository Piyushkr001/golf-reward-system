...
export default function AdminSignInPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Additional safety check based on return data
    ...
    <div className="w-full max-w-md bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-rose-200 dark:border-rose-900/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
      ...
      <form onSubmit={handleLogin} className="flex flex-col gap-6">
        ...
        <Button disabled={loading} type="submit" size="lg" className="w-full h-12 bg-slate-900 dark:bg-white hover:opacity-90 text-white dark:text-slate-900 rounded-xl shadow-lg transition-all text-base font-semibold">
          {loading ? <Loader2 className="animate-spin h-5 w-5 mx-auto" /> : 'Sign In as Admin'}
        </Button>
      </form>

      <p className="mt-8 text-center text-sm text-slate-600 dark:text-slate-400">
        Need administrator access?{' '}
        <Link href="/admin/sign-up" className="font-semibold text-rose-600 dark:text-rose-400 hover:underline">
          Request Here
        </Link>
      </p>
    </div>
